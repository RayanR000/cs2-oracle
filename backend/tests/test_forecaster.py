...
class TestPlayerCountFeatures:
    def test_fetch_player_counts_empty_when_no_parquet(self, forecaster):
        with patch("pathlib.Path.exists", return_value=False):
            df = forecaster._fetch_player_counts()
        assert df.empty
        for col in ("day", "mean_players", "peak_players", "reading_count"):
            assert col in df.columns

    def test_add_player_count_features_adds_columns(self, forecaster):
        ...
        with patch.object(forecaster, "_fetch_player_counts", return_value=pd.DataFrame({
            "day": pd.to_datetime(["2026-03-01", "2026-03-02"]),
            "mean_players": [500000, 510000],
            "peak_players": [550000, 560000],
            "min_players": [450000, 460000],
            "reading_count": [12, 12],
            "last_players": [505000, 512000],
            "date": [date(2026, 3, 1), date(2026, 3, 2)],
        })):
            result = forecaster._add_player_count_features(input_df)
        expected_cols = [
            "players_mean", "players_peak", "players_min",
            "players_last", "players_readings",
            "players_change_1d", "players_change_7d",
            "players_z_score_30d", "players_ma7", "players_mean_ratio_7d",
        ]
        for col in expected_cols:
            assert col in result.columns, f"Missing column: {col}"
        assert result["players_mean"].iloc[0] == 500000.0
        assert result["players_peak"].iloc[0] == 550000.0
        assert result["players_min"].iloc[0] == 450000.0

    def test_add_player_count_features_fills_zeros_when_empty(self, forecaster):
        input_df = pd.DataFrame({
            "item_id": ["a"],
            "date": [date(2026, 3, 1)],
            "price": [100.0],
    ...
    def test_change_features_computed_correctly(self, forecaster):
        input_df = pd.DataFrame({
            "item_id": ["a"] * 10,
            "date": [date(2026, 1, 1) + timedelta(days=i) for i in range(10)],
            "price": [100.0] * 10,
        ...
        pc_df = pd.DataFrame({
            "day": pd.to_datetime([date(2026, 1, 1) + timedelta(days=i) for i in range(10)]),
            "mean_players": [float(100000 + i * 1000) for i in range(10)],
            "peak_players": [float(110000 + i * 1000) for i in range(10)],
            "min_players": [float(90000 + i * 1000) for i in range(10)],
            "reading_count": [12] * 10,
            "last_players": [float(105000 + i * 1000) for i in range(10)],
            "date": [date(2026, 1, 1) + timedelta(days=i) for i in range(10)],
        })
        with patch.object(forecaster, "_fetch_player_counts", return_value=pc_df):
            result = forecaster._add_player_count_features(input_df)
        # players_change_1d should be ~1000 between consecutive rows
        assert result["players_change_1d"].iloc[1] == pytest.approx(1000.0, abs=1)
        assert result["players_change_1d"].iloc[0] != result["players_change_1d"].iloc[0]
        # first row change is NaN (shift before fill)
        assert pd.isna(result["players_change_1d"].iloc[0]) or result["players_change_1d"].iloc[0] == 0.0

    def test_ma7_and_z_score_available(self, forecaster):
        input_df = pd.DataFrame({
            "item_id": ["a"] * 60,
            "date": [date(2026, 1, 1) + timedelta(days=i) for i in range(60)],
            "price": [100.0] * 60,
            "volume": [50] * 60,
        })
        pc_df = pd.DataFrame({
            "day": pd.to_datetime([date(2026, 1, 1) + timedelta(days=i) for i in range(60)]),
            "mean_players": [float(100000 + (i % 30) * 500) for i in range(60)],
            "peak_players": [float(110000 + (i % 30) * 500) for i in range(60)],
            "min_players": [float(90000 + (i % 30) * 500) for i in range(60)],
            "reading_count": [12] * 60,
            "last_players": [float(105000 + (i % 30) * 500) for i in range(60)],
            "date": [date(2026, 1, 1) + timedelta(days=i) for i in range(60)],
        })
        with patch.object(forecaster, "_fetch_player_counts", return_value=pc_df):
            result = forecaster._add_player_count_features(input_df)
        assert "players_ma7" in result.columns
        assert "players_z_score_30d" in result.columns
        assert "players_mean_ratio_7d" in result.columns
        # MA7 should be non-negative
        assert (result["players_ma7"].dropna() >= 0).all()
        # Z-score should be finite
        assert result["players_z_score_30d"].dropna().apply(lambda x: np.isfinite(x)).all()
