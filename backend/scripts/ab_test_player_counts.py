#!/usr/bin/env python3
"""
A/B test: compare prediction accuracy WITH vs WITHOUT player count features.

Trains two models on the same expanding-window walk-forward evaluation:
  - Model A (control):  no player count features
  - Model B (treatment): with player count features

Prints a side-by-side comparison table.

Usage:
    python scripts/ab_test_player_counts.py [--max-items 200]
"""
...
logger = logging.getLogger("ab_test_player_counts")

ARCHIVE_DIR = Path(__file__).parent.parent.parent / "price-archive"


def run_evaluation(max_items=200, use_player_counts=True):
    """Walk-forward evaluation, optionally including player count features.

    Mirrors evaluate_forecaster.py methodology identically.
    """
    import duckdb
    con = duckdb.connect()
    db = SessionLocal()
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("A/B TEST: Player Count Features")
    logger.info("=" * 70)

    # ── Run WITHOUT player counts ──────────────────────────────────
    logger.info("\n>>> MODEL A (CONTROL): Without player count features <<<")
    results_without = run_evaluation(max_items=args.max_items, use_player_counts=False)

    # ── Run WITH player counts ─────────────────────────────────────
    logger.info("\n>>> MODEL B (TREATMENT): With player count features <<<")
    results_with = run_evaluation(max_items=args.max_items, use_player_counts=True)

    # ── Compare ────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("A/B TEST RESULTS — Player Count Features")
    print("=" * 70)
    ...
    print(f"\n  Verdict: Player count features → {summary} ({avg_delta:+.2f}pp avg)")
